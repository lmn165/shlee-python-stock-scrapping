...
if __name__ == "__main__":
    baseurl = (
        ...
        "article_id=0000673432&office_id=031&mode=search&query=amd&page=1"
    )
    nametag = "articleCont"
    resultarray = []
    ...
    # print(idxname.text)
    date = datetime.datetime.strptime(pagehtmlbs.find("span", class_="article_date").text, "%Y-%m-%d %H:%M").strftime(
        "%Y%m%d%H%M"
    )
